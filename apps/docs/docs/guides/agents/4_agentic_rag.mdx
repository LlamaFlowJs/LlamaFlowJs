...
To learn more about RAG, we recommend this [introduction](https://docs.llamaflowjs.ai/en/stable/getting_started/concepts/) from our Python docs. We'll assume you know the basics:
...
We'll be bringing in `SimpleDirectoryReader`, `HuggingFaceEmbedding`, `VectorStoreIndex`, and `QueryEngineTool` from llamaflowjs, as well as the dependencies we previously used.
...
Before llamaflowjs can send a query to the LLM, it needs to find the most relevant chunks to send. That's the purpose of a `Retriever`. We're going to get `VectorStoreIndex` to act as a retriever for us
...
By default llamaflowjs will retrieve just the 2 most relevant chunks of text. This document is complex though, so we'll ask for more context.
