...
LlamaParse is an API created by llamaflowjs to efficiently parse files, e.g. it's great at converting PDF tables into markdown.

To use it, first login and get an API key from https://cloud.llamaflowjs.ai. Make sure to store the key as `apiKey` parameter or in the environment variable `LLAMA_CLOUD_API_KEY`.

Official documentation for LlamaParse can be found [here](https://docs.cloud.llamaflowjs.ai/).

## Usage

You can then use the `LlamaParseReader` class to load local files and convert them into a parsed document that can be used by llamaflowjs.
See [LlamaParseReader.ts](https://github.com/LlamaFlowJs/LlamaFlowJs/blob/main/packages/llamaflowjs/src/readers/LlamaParseReader.ts) for a list of supported file types:
...
- `parsingInstructions?` Optional. Can help with complicated document structures. See this [llamaflowjs Blog Post](https://www.llamaflowjs.ai/blog/launching-the-first-genai-native-document-parsing-platform) for an example.
